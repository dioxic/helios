...
import uk.dioxic.mgenerate.ksp.model.DefaultValue
import java.io.File
...
object DefaultParameterValueReader {
    ...
    private fun importedDefaultValue(
        ...
        var result = auxText.trim()
        if (unquotedText(result).contains("(")) {
            result = result.defaultValueCodeWithFunctionCalls()
        } else {
            val index = result.indexOfFirstIgnoreQuoted { it == ',' || it == '\n' || it == ')' }
